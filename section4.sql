...
-- left join users and photo table
SELECT url, username
FROM photos
LEFT JOIN users ON users.id = photos.user_id;

-- adding new data in users table 
INSERT INTO users (username)
VALUES ('Nicole');

-- right join photos and users table
SELECT url, username
FROM photos
RIGHT JOIN users ON users.id = photos.user_id;

-- full join photos and users table
SELECT url, username
FROM photos
...
-- Advanced Join (comments, photos, user table)
...
SELECT user_id, COUNT(id) AS num_comment_created
FROM comments
GROUP BY user_id;
...
-- find how much comments in photo
...
-- Find the users (user_ids) where the users has commented on the first 50 photos and the users added more than 20 comments on those photos
SELECT user_id, COUNT(*)
FROM comments
WHERE photo_id < 50
GROUP BY user_id
